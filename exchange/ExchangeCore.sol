   ...
   * @dev Approve an order and optionally mark it for orderbook inclusion. Must be called by the maker of the order
    ...
    * @dev Calculate the price two orders would match at, if in fact they would match (otherwise fail)
    ...
    returns (uint)
    {
        ...
        /* Require price cross. */
        require(buyPrice >= sellPrice, "CATGIRL_Exchange::Buy price must greater than sell price!");
    ...
    returns (uint)
    {
        ...
        if (sell.paymentToken != address(0)) {
            require(msg.value == 0, "CATGIRL_Exchange::Redundant sent funds!");
        }
        ...
        if (sell.feeRecipient != address(0)) {
            ...
            require(sell.takerRelayerFee <= buy.takerRelayerFee, "CATGIRL_Exchange::taker fee is more than to maximum fee specified by buyer");

            if (sell.makerRelayerFee > 0) {
                uint makerRelayerFee =  sell.makerRelayerFee * price /INVERSE_BASIS_POINT;
                if (sell.paymentToken == address(0)) {
                    receiveAmount = receiveAmount - makerRelayerFee;
                    require(payable(sell.feeRecipient).send(makerRelayerFee), "CATGIRL_Exchange::Fee transfer to maker failed!");
                } else {
            ...
            if (sell.takerRelayerFee > 0) {
                ...
                if (sell.paymentToken == address(0)) {
                    ...
                    require(payable(sell.feeRecipient).send(takerRelayerFee), "CATGIRL_Exchange::Fee transfer to taker failed!");
                } else {
    ...
    * @dev Atomically match two orders, ensuring validity of the match, and execute all associated state transitions. Protected against reentrancy by a contract-global lock.
    ...
    nonReentrant
    {
        ...
        if (buy.maker == msg.sender) {
            require(validateOrderParameters(buy), "CATGIRL_Exchange::Invalid buy order params!");
        } else {
        ...
        if (sell.maker == msg.sender) {
            require(validateOrderParameters(sell), "CATGIRL_Exchange::Invalid sell order params!");
        } else {
        ...
        /* Must be matchable. */
        require(ordersCanMatch(buy, sell), "CATGIRL_Exchange::Order not matched");
        ...
        require(size > 0, "CATGIRL_Exchange::Order target is not a contract!");
        ...
        require(ArrayUtils.arrayEq(buy.callData, sell.callData), "CATGIRL_Exchange::Calldata after replacement is invalid!");
        ...
        require(success, "CATGIRL_Exchange::ERC721 Transfer failed!");
